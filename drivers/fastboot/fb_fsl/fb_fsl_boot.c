...
void
bootimg_print_image_hdr(struct andr_boot_img_hdr_v0 *hdr)
{
#ifdef DEBUG
	int i;
...
static struct andr_boot_img_hdr_v0 boothdr __aligned(ARCH_DMA_MINALIGN);
#endif
...
int do_boota(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	char *ptn = "boot";
	int mmcc = -1;
	struct andr_boot_img_hdr_v0 *hdr = &boothdr;
	ulong image_size;
	bool check_image_arm64 =  false;
	int i = 0;
	...
	if (mmcc != -1) {
		...
		image_size = android_image_get_end(hdr, vendor_boot_hdr) - (ulong)hdr;
